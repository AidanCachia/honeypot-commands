...
This file documents the resource monitoring commands used for the traditional Cowrie honeypot. These commands were used to record VMStat output and calculate peak CPU usage, peak RAM usage, and lowest free memory during testing.
...
## 7. Calculating Peak RAM Usage
The following command was used to calculate peak RAM usage from the Cowrie VMStat log. The calculation assumes the VM had 4096 MB of allocated RAM:
```bash
awk '$1 ~ /^[0-9]+$/ {used=(4096-($4/1024)); pct=(used/4096)*100; print pct}' ~/monitor_logs/cowrie_vmstat.log | sort -nr | head -1 | awk '{printf("Peak RAM Usage: %.1f%%\n",$1)}'
```
Example output:
```text
Peak RAM Usage: 37.6%
...
These commands were used to record and analyse the resource usage of the traditional Cowrie honeypot during testing. The values collected from this process were later used to compare Cowrie's resource usage with the LLM-based honeypot.
